...
const Home: React.FC = () => {
  ...
  const checkForUpdate = async () => {
    ...
    try {
      ...
      const message = formatWebRtcError(updateError)
      setUpdate((current) => ({ ...current, checking: false, status: '检查更新失败' }))
      setError(message)
    }
  }
  ...
  useEffect(() => {
    ...
    const removeError = window.appUpdate.onError((message) => {
      setUpdate((current) => ({ ...current, checking: false, installing: false, status: '更新失败' }))
      setError(message)
    })
    return () => {
      removeProgress()
