...
import { Alert, Button, Input, Modal, Popover, Select, Slider, Switch, Tooltip } from 'antd'
import {
  ConnectionState,
  LocalAudioTrack,
  Participant,
...
import { useClientStore, type AudioPreferences } from './stores/client'
...
function ParticipantVolumePopover({ menu, volume, muted, onChange, onToggleMuted, onReset }: ParticipantVolumePopoverProps) {
  ...
  return <div className="participant-volume-menu" style={{ left: menu.x, top: menu.y }} onClick={(event) => event.stopPropagation()} onContextMenu={(event) => event.preventDefault()}><div className="participant-volume-heading"><span className="participant-volume-person"><Tooltip title={muteAction}><Button type={muted ? 'primary' : 'text'} danger={muted} shape="circle" size="small" aria-label={`${muteAction}${menu.name}`} onClick={onToggleMuted} icon={muted ? <VolumeX size={15} /> : <Volume2 size={15} />} /></Tooltip><span className="participant-volume-name">{menu.name}</span></span><output>{muted ? '已静音' : `${volume}%`}</output></div><Slider aria-label={`${menu.name}的音量`} min={0} max={100} value={volume} onChange={(nextValue) => onChange(typeof nextValue === 'number' ? nextValue : nextValue[0])} /><div className="participant-volume-footer"><span>仅影响你听到的声音</span><Button type="link" onClick={onReset}>恢复默认</Button></div></div>
}
...
function AudioControlButton({ kind, muted, volume, onToggle, onVolumeChange }: AudioControlButtonProps) {
  ...
  return <Popover placement="top" trigger="hover" title={`${channelName} · ${muted ? '已关闭' : '已开启'}`} content={<div className="room-volume-popover"><div className="room-volume-heading"><span>音量</span><strong>{volume}%</strong></div><Slider aria-label={`${channelName}音量`} min={0} max={100} value={volume} onChange={(nextValue) => onVolumeChange(typeof nextValue === 'number' ? nextValue : nextValue[0])} /></div>}><Button type={muted ? 'primary' : 'default'} danger={muted} shape="circle" size="large" aria-label={actionName} onClick={onToggle} icon={icon} /></Popover>
}
